...
namespace App.Helpers
{
    ...
    public static class FriendlyUrlHelper
    {
        ...
        public static string[] GetFriendlyUrlParameter(string title, bool remapToAscii = false, int maxlength = 80)
        {
            ...
            for (int i = 0; i < length; ++i)
            {
                ...
                else
                {
                    ...
                    if (remapToAscii)
                    {
                        stringBuilder.Append(RemapUrlParameter(c));
                    }
                    else
                    {
                        stringBuilder.Append(c);
            ...
            if (prevdash)
            {
                return stringBuilder.ToString().Substring(0, stringBuilder.Length - 1).Split(',').ToArray<string>();
            }
            else
            {
                return stringBuilder.ToString().Split(',').ToArray<string>();
            }
        }
        private static string RemapUrlParameter(char character)
        {
            string s = character.ToString().ToLowerInvariant();
            if (character == '_')
            {
